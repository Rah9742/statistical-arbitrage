...
portfolio_1_tickers = np.array(["INTU"])
portfolio_2_tickers = np.array(["TEAM"])

# print(f"https://api.polygon.io/v2/aggs/ticker/FDX/range/1/day/{trading_dates[0]}/{trading_dates[-1]}?adjusted=true&sort=asc&limit=50000&apiKey={polygon_api_key}")

portfolio_1_data_list = []
portfolio_2_data_list = []

def create_stock_date(portfolio_tickers, portfolio_data_list):

    for portfolio_stock in portfolio_tickers:
    
        stock_data = pd.json_normalize(requests.get(f"https://api.polygon.io/v2/aggs/ticker/{portfolio_stock}/range/1/day/{trading_dates[0]}/{trading_dates[-1]}?adjusted=true&sort=asc&limit=50000&apiKey={polygon_api_key}").json()["results"]).set_index("t")
        # print(stock_data)
        ...
        stock_data["ticker"] = portfolio_stock
        stock_data = stock_data[["c", "pct_change", "ticker"]].dropna()
        
        portfolio_data_list.append(stock_data)

create_stock_date(portfolio_1_tickers, portfolio_1_data_list)
create_stock_date(portfolio_2_tickers, portfolio_2_data_list)
...
portfolio_1_data["port_perf"] = portfolio_1_data["pct_change"].cumsum()*100
# Console().print(portfolio_1_data.head().to_markdown())
...
portfolio_2_data["port_perf"] = portfolio_2_data["pct_change"].cumsum()*100
# Console().print(portfolio_2_data.head().to_markdown())
...
# Console().print(combined_portfolio_data.head().to_markdown())
combined_portfolio_data["spread"] = abs(combined_portfolio_data["1_port_perf"] - combined_portfolio_data["2_port_perf"])
# Console().print(combined_portfolio_data.head().to_markdown())

# Test p-value to ensure stationarity | the closer to < 0.05, the better
p_value = check_cointegration(combined_portfolio_data['1_port_perf'].values, combined_portfolio_data['2_port_perf'].values)
...
combined_portfolio_data["underperformer_price"] = combined_portfolio_data.apply(lambda row: row['1_c'] if (row['2_port_perf'] > row['1_port_perf']) else row['2_c'], axis=1)
combined_portfolio_data["overperformer_price"] = combined_portfolio_data.apply(lambda row: row['1_c'] if (row['2_port_perf'] < row['1_port_perf']) else row['2_c'], axis=1)
...
plt.title(f"{portfolio_1_tickers[0]}-{portfolio_2_tickers[0]} Cointegration Strategy")
...
plt.figure(dpi=200)
plt.xticks(rotation=45)
plt.title(f"{portfolio_1_tickers[0]}-{portfolio_2_tickers[0]} Return Spread")
...
plt.figure(dpi=200)
plt.xticks(rotation=45)
plt.title(f"{portfolio_1_tickers[0]}-{portfolio_2_tickers[0]} Spread")
plt.plot(combined_portfolio_data.index, combined_portfolio_data["spread"])
...
plt.figure(dpi=200)
plt.xticks(rotation=45)
plt.title(f"{portfolio_1_tickers[0]}-{portfolio_2_tickers[0]} Return Performance")
plt.plot(combined_portfolio_data.index, combined_portfolio_data["1_port_perf"])
plt.plot(combined_portfolio_data.index, combined_portfolio_data["2_port_perf"])
plt.legend([portfolio_1_tickers[0], portfolio_2_tickers[0]])
plt.show()
